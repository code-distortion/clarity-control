...
// do the same, but with extra configuration
...
Clarity Control is a part of the ***Clarity Suite***:
- **Clarity Control** - Handle Your Exceptions
...
- [Logging Exceptions (Advanced)](#logging-exceptions-advanced)
  - [The Context Object](#the-context-object)
  - [Context Object Without an Exception](#context-object-without-an-exception)
...
Some settings require a logger that's aware of Clarity (such as [code-distortion/clarity-logger](https://github.com/code-distortion/clarity-logger)) to work. Specifically, the *channel*, *log level*, and *"known"* issue settings won't work otherwise.

These details are added to a `Context` object that's produced when an exception occurs. It's up to the Logger code to read and use the Context objects' values.

See [code-distortion/clarity-context](https://github.com/code-distortion/clarity-context#logging-exceptions) for more information about this `Context` object.

See [code-distortion/clarity-logger](https://github.com/code-distortion/clarity-logger) for a logger that understands the `Context` object.



## Log Channel

Specify which Laravel log-channel you'd like to log to. The possible values come from your projects' `config/logging.php` file.
...
You can specify a default value to return when an exception occurs by passing a second parameter to `Control::run()` or `Control::prepare()`.
