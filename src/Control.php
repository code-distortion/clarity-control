...
class Control
{
     ...
     * @param boolean  $instantiatedUsingRun Whether the caller called run (which runs execute() straight away) or not.
     ...
     */
    public static function run(callable $callable, mixed $default = null): mixed
    {
        $defaultWasSpecified = func_num_args() >= 2;

        return (new self())->init($callable, true, $default, $defaultWasSpecified)->execute();
    }
     ...
     * @throws Throwable Exceptions that weren't supposed to be caught.
     */
    public static function prepare(callable $callable, mixed $default = null): self
    {
        $defaultWasSpecified = func_num_args() >= 2;

        return (new self())->init($callable, false, $default, $defaultWasSpecified);
    }
